...
export default function EditTeacherPage({
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (!teacherRes.ok || !classesRes.ok || !subjectsRes.ok) {
          throw new Error("Failed to load data")
        }
        const teacher: TeacherData = await teacherRes.json()

        const classesData: ClassOption[] = await classesRes.json()
        const subjectsData: SubjectOption[] = await subjectsRes.json()
        ...
        reset({
          ...
          dateEmployed: teacher.dateEmployed ? new Date(teacher.dateEmployed).toISOString().split("T")[0] : "",
        ...
        toast({
          title: "Error",
          description: err instanceof Error ? err.message : "Failed to load teacher data",
          variant: "destructive",
        })
      } finally {
        setLoading(false)
  ...
  function toggleSubject(subjectId: string) {
    ...
    if (current.includes(subjectId)) {
      setValue("subjectIds", current.filter((s) => s !== subjectId))
    } else {
  ...
  async function onSubmit(data: TeacherInput) {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || "Failed to update teacher")
      }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Edit Teacher" description="Update teacher information">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid gap-6 sm:grid-cols-2">
              <FormField label="Staff ID" error={undefined}>
                <Input {...register("staffId")} disabled className="text-muted-foreground" />
              </FormField>
            ...
            <div className="flex gap-3 pt-4 border-t">
              ...
              <Button variant="outline" asChild>
                <Link href={`/teachers/${id}`}>Cancel</Link>
              </Button>
            </div>
          </form>
        </CardContent>
